...
const revenueRoutes: FastifyPluginAsync = async (app) => {
    ...
    app.get("/v1/revenue/expiring-licenses", async (req, reply) => {
        ...
        const filtered = licenses.filter((lic: any) => {
            const alreadySent = lic.emailLogs.some((log: any) => log.emailType === emailTypeForDays);
            return !alreadySent;
        });
    ...
    app.get("/v1/revenue/failed-payments", async (req, reply) => {
        ...
        const results = failedPayments.map((event: any) => {
    ...
    app.get("/v1/revenue/upsell-candidates", async (req, reply) => {
        ...
        const filtered = candidates.filter((lic: any) => {
        ...
        return reply.send({
            count: filtered.length,
            candidates: filtered.map((lic: any) => ({
                licenseId: lic.id,
                userId: lic.userId,
                email: lic.user.email,
